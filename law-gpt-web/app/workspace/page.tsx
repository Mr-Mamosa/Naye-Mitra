...
export default function LawGPTInterface() {
  ...
  const fetchSidebarCases = async () => {
    try {
      const response = await fetch("https://https://immutable-ezra-unvoting.ngrok-free.dev/api/v1/cases/lawyer_1");
      if (response.ok) {
  ...
  const fetchCaseMessages = async (caseId: string) => {
    try {
      const response = await fetch(`https://https://immutable-ezra-unvoting.ngrok-free.dev/api/v1/case/${caseId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleDeleteCase = async (caseId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const response = await fetch(`https://immutable-ezra-unvoting.ngrok-free.dev/api/v1/case/${caseId}`, { method: "DELETE" });
      if (response.ok) {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch("https://immutable-ezra-unvoting.ngrok-free.dev/api/v1/consult", {
        method: "POST",
